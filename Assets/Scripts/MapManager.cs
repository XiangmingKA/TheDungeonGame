...
public class MapManager : MonoBehaviour {
    ...
    void CreateRoom()
    {
        for (int i = 0; i < Size * 2; i++)
        {
            for (int j = 0; j < Size * 2; j++)
            {
                ...
                GameObject go = Instantiate(ChooseRoomPrefab(map[i, j]), map[i, j].pos * roomSize, Quaternion.identity);//生成房间的函数调用
                go.transform.SetParent(GameObject.Find("GO").transform);
            }
        }
    }

    GameObject ChooseRoomPrefab(Node node)
    {
        //中心节点
    ...
    private void CreateCrossRoom()//叶子节点房间创建成十字路口房间
    {
